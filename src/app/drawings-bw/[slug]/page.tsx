...
export function generateStaticParams() {
  return drawingsBWProjects.map((project) => ({ slug: project.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const project = drawingsBWProjects.find((entry) => entry.slug === slug);
  ...
  const url = `/drawings-bw/${project.slug}`;
...
export default async function ArchitectureProjectDetailPage({ params }: PageProps) {
  ...
  const projectUrl = `https://dsrenders.com/drawings-bw/${project.slug}`;
  ...
  const relatedProjects = drawingsBWProjects
